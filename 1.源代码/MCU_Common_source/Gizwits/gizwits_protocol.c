...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flaganti_theft_mode)
    {
        bitFieldBitLen += anti_theft_mode_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaganti_theft_mode)
    {
        info->event[info->num] = EVENT_anti_theft_mode;
        info->num++;
        dataPoints->valueanti_theft_mode = gizVarlenDecompressionValue(bitFieldOffset,anti_theft_mode_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += anti_theft_mode_LEN;
    }


    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtempValue)
    {
        info->event[info->num] = EVENT_tempValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetempValue,(uint8_t *)&issuedData[byteFieldOffset],tempValue_LEN);
        byteFieldOffset += tempValue_LEN;
        dataPoints->valuetempValue = gizX2Y(tempValue_RATIO,  tempValue_ADDITION, elongateP0FormTmp.devStatus.valuetempValue); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaghumiValue)
    {
        info->event[info->num] = EVENT_humiValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuehumiValue,(uint8_t *)&issuedData[byteFieldOffset],humiValue_LEN);
        byteFieldOffset += humiValue_LEN;
        dataPoints->valuehumiValue = gizX2Y(humiValue_RATIO,  humiValue_ADDITION, elongateP0FormTmp.devStatus.valuehumiValue); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagSmogeValue)
    {
        info->event[info->num] = EVENT_SmogeValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueSmogeValue,(uint8_t *)&issuedData[byteFieldOffset],SmogeValue_LEN);
        byteFieldOffset += SmogeValue_LEN;
        dataPoints->valueSmogeValue = gizX2Y(SmogeValue_RATIO,  SmogeValue_ADDITION, elongateP0FormTmp.devStatus.valueSmogeValue); 
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagCO2Value)
    {
        ...
        dataPoints->valueCO2Value = gizX2Y(CO2Value_RATIO,  CO2Value_ADDITION, elongateP0FormTmp.devStatus.valueCO2Value); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuefire != cur->valuefire)
    {
        ...
        gizwitsProtocol.waitReportDatapointFlag.flagfire = 1;
        ret = 1;
    }
    if(last->valuevalue != cur->valuevalue)
    {
        GIZWITS_LOG("valuevalue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagvalue = 1;
        ret = 1;
    }
    ...
    if(last->valueanti_theft_mode != cur->valueanti_theft_mode)
    {
        GIZWITS_LOG("valueanti_theft_mode Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaganti_theft_mode = 1;
        ret = 1;
    }
    if(last->valuetempValue != cur->valuetempValue)
    {
        GIZWITS_LOG("valuetempValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempValue = 1;
        ret = 1;
    }
    if(last->valuehumiValue != cur->valuehumiValue)
    {
        GIZWITS_LOG("valuehumiValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaghumiValue = 1;
        ret = 1;
    }
    if(last->valueSmogeValue != cur->valueSmogeValue)
    {
        GIZWITS_LOG("valueSmogeValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagSmogeValue = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagfire)
    {
        if(dataPoints->valuefire >= fire_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valuefire Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,fire_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuefire);
                bitFieldOffset += fire_LEN;
        }
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagvalue)
    {
        if(dataPoints->valuevalue >= value_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valuevalue Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,value_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuevalue);
                bitFieldOffset += value_LEN;
        }
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flaganti_theft_mode)
    {
        if(dataPoints->valueanti_theft_mode >= anti_theft_mode_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valueanti_theft_mode Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,anti_theft_mode_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueanti_theft_mode);
                bitFieldOffset += anti_theft_mode_LEN;
        }
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempValue)
    {
        devStatusTmp.valuetempValue = gizY2X(tempValue_RATIO,  tempValue_ADDITION, dataPoints->valuetempValue); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetempValue,tempValue_LEN);
        byteFieldOffset += tempValue_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaghumiValue)
    {
        devStatusTmp.valuehumiValue = gizY2X(humiValue_RATIO,  humiValue_ADDITION, dataPoints->valuehumiValue); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuehumiValue,humiValue_LEN);
        byteFieldOffset += humiValue_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagSmogeValue)
    {
        devStatusTmp.valueSmogeValue = gizY2X(SmogeValue_RATIO,  SmogeValue_ADDITION, dataPoints->valueSmogeValue); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueSmogeValue,SmogeValue_LEN);
        byteFieldOffset += SmogeValue_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagCO2Value)
    {
        devStatusTmp.valueCO2Value = gizY2X(CO2Value_RATIO,  CO2Value_ADDITION, dataPoints->valueCO2Value); 
