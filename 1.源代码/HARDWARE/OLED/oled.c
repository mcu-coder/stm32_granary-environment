...
// 封装清除指定区域的函数（按需调用）
void OLED_ClearArea(u8 x_start, u8 x_end, u8 y_start, u8 y_end)
{
    u8 i, page;
    // 遍历y轴对应的page（OLED按8行一页存储，16行对应2个page）
    for(page = y_start/8; page <= (y_end-1)/8; page++)
    {
        // 遍历x轴范围
        for(i = x_start; i <= x_end; i++)
        {
            OLED_GRAM[i][page] = 0; // 清空对应GRAM位置
        }
    }
    OLED_Refresh(); // 刷新显示
}
